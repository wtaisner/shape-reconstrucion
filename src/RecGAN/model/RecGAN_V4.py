...
class RecGAN(nn.Module):
    def __init__(self):
        ...
        self.unet = ResidualUNet3D(1, 1, final_sigmoid=True, f_maps=64, layer_order='gcr', num_groups=8, num_levels=5,
                                   is_segmentation=True, conv_padding=1)
        # create discriminator (like the encoder)
        self.discriminator = Discriminator(1, 1, basic_module=ExtResNetBlock, final_sigmoid=True, f_maps=8,
                                           layer_order='gcr', num_groups=2, num_levels=7, is_segmentation=False,
                                           conv_padding=1)
